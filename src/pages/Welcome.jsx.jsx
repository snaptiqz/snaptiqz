...
const Welcome = () => {
  ...
  const { user, setJustSignedUp, useSession } = useContext(AuthContext);

  const { isLoading } = useSession(); // ensures session is fetched

  useEffect(() => {
    if (isLoading) return; // don't trigger until session is loaded
  ...
  }, [navigate, setJustSignedUp, isLoading]);
